import { collectingCustomerService } from '*/services/Customer/collectingCustomer.service'
...
const getCollectingByName = async (req, res) => {
    try {
        ...
        const result = await collectingCustomerService.getCollectingByName(name)
        res.status(HttpStatusCode.OK).json(result)
    } catch (error) {
        res.status(HttpStatusCode.INTERNAL_SERVER).json({
            error: error.message
...
const getFullCollection = async (req, res) => {
    try {
        const result = await collectingCustomerService.getFullCollection()
        res.status(HttpStatusCode.OK).json(result)
    } catch (error) {
        res.status(HttpStatusCode.INTERNAL_SERVER).json({
            error: error.message
...
export const collectingCustomerController = { 
    getFullCollection,
    getCollectingByName
    // getFullUser, 
    // update 
